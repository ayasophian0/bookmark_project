...
async def extract_youtube_text(url):
...
async def extract_with_playwright(url):
    try:
        async with async_playwright() as p:
            ...
            await browser.close()
        return await asyncio.to_thread(trafilatura.extract, content)
    except:
        return None
...
async def extract_text_smart(url):
    if is_youtube_url(url):
        text = await extract_youtube_text(url)
        if text:
            return text, "youtube"
        else:
            return None, "youtube_failed"

    text = await extract_with_trafilatura(url)
...
async def analyze_content(text, url, force_video=False):
    if not client:
        return "{}", 0, 0
...
async def analyze_url_only(url):
    if not client:
        return "{}", 0, 0
...
async def normalize_categories(dataset):
...
async def process_single_url(url, sem):
    async with sem:
        source = get_url_source(url)
        try:
            text, method = await extract_text_smart(url)
            
            if text:
                force_video = (method == "youtube")
                analysis_raw, p_tokens, c_tokens = await analyze_content(text, url=url, force_video=force_video)
                ...
                analysis_raw, p_tokens, c_tokens = await analyze_url_only(url)
                parsed = safe_parse_json(analysis_raw)
                ctype = str(parsed.get("content_type", "Text")).title()
                if ctype not in ["Text", "Video", "Audio"]:
                    ctype = "Text"
        ...
        if unique_urls_to_process:
            ...
            with st.status("Extracting and Analyzing...") as status:
                ...
                async def process_all():
                    ...
                    tasks = [asyncio.create_task(process_single_url(url, sem)) for url in unique_urls_to_process]
                    completed_count = 0
                    ...
                    for coro in asyncio.as_completed(tasks):
                        ...
                        if res.get("success"):
                            method = res["method"]
                            entry_url = res["entry"]["url"]
                            if method == "url_inference":
                                st.write(f"⚠️ Extracted via URL inference: {entry_url}")
                            else:
                                st.write(f"✅ Success ({method}): {entry_url}")
                            ...
                            failed_url = res.get("url", "Unknown URL")
                            st.write(f"❌ Failed processing {failed_url}: {res.get('error')}")
                    
                    st.write("🤖 Normalizing categories...")
                    st.session_state.dataset = await normalize_categories(st.session_state.dataset)
